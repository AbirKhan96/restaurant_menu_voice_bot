...
st.sidebar.title("Language Settings")
language = st.sidebar.selectbox("Choose language", ["English", "Hindi", "Arabic"])
...
if st.session_state.messages[-1]["role"] != "assistant":
    with st.chat_message("assistant"):
        with st.spinner("Thinking🤔..."):
            # Query the LLaMA-based RAG model
            documents = SimpleDirectoryReader("/home/aiml/Abir/Useful_CODE/llm/submit_proposal_cap1/data/").load_data()
            system_prompt = f"""You are a Q&A assistant. Your goal is to answer questions as accurately as possible based on the instructions and context provided. Respond only to the current query and avoid including extra information or unrelated answers. Respond in {language}."""
            query_wrapper_prompt = SimpleInputPrompt("<|USER|>{query_str}<|ASSISTANT|>")
            llm = HuggingFaceLLM(
                context_window=4096,
                max_new_tokens=256,
