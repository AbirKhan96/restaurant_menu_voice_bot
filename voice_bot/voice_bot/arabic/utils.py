...
def get_answer(messages, language):
    system_message = [{"role": "system", "content": f"You are a helpful bilingual AI chatbot.You are a helpful bilingual AI chatbot. Respond only to the current question and avoid including extra information or unrelated answers. Respond in {language}."}]
    messages = system_message + messages
    response = client.chat.completions.create(
        model="gpt-3.5-turbo-1106",
        messages=messages
...
def speech_to_text(audio_data, language):
    # Set the language code based on the input language
    lang_code = "en" if language == "English" else "hi" if language == "Hindi" else "ar" if language == "Arabic" else None
...
def text_to_speech(input_text, language):
    try:
        ...
        if language == "English":
            ...
            voice = "fable"  # Updated to 'fable' for Arabic
        else:
            raise ValueError(f"Unsupported language: {language}")
